...
const statusClass = computed(() => {
  ...
  if (member.value.deafened) return 'deafened'
  if (member.value.muted) return 'muted'
  if (member.value.speaking) return 'speaking'
  return 'online'
})

const statusIcon = computed(() => {
  if (isOffline.value) return Warning
  if (member.value.deafened) return Headset
  if (member.value.muted) return Mute
  return Microphone
})

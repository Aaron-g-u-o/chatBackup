...
public class MsgSendConsumer implements RocketMQListener<MsgSendMessageDTO> {
    ...
    public void onMessage(MsgSendMessageDTO dto) {
        ...
        if (message == null) {
            log.warn("consume message failed: message not found, msgId: {}", dto.getMsgId());
            return;
        }

        Room room = roomCache.get(message.getRoomId());
        if (room == null) {
            log.warn("consume message failed: room not found, roomId: {} for msgId: {}", message.getRoomId(), message.getId());
            return;
        }

        ChatMessageResp msgResp = chatService.getMsgResp(message, null);
        if (msgResp == null) {
            log.warn("consume message failed: build ChatMessageResp returned null for msgId: {}", message.getId());
            return;
        }
        ...
        if (room.isHotRoom()) {//热门群聊推送所有在线的人
            ...
            if (Objects.equals(room.getType(), RoomTypeEnum.GROUP.getType())) {//普通群聊推送所有群成员
                ...
                if (roomFriend == null) {
                    log.warn("consume message failed: roomFriend not found for roomId: {}", room.getId());
                    return;
                }

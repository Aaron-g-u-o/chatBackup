...
public class RoomAppServiceImpl implements RoomAppService {
    ...
    private List<ChatRoomResp> buildContactResp(Long uid, List<Long> roomIds) {
        ...
        return roomBaseInfoMap.values().stream().map(room -> {
                    ...
                    if (Objects.nonNull(message)) {
                        ...
                        User fromUser = lastMsgUidMap.get(message.getFromUid());
                        String fromName = fromUser == null ? "" : (fromUser.getName() == null ? "" : fromUser.getName());
                        String content = strategyNoNull.showContactMsg(message);
                        if (fromName.isEmpty()) {
                            resp.setText(content == null ? "" : content);
                        } else {
                            resp.setText(fromName + ":" + (content == null ? "" : content));
                        }
                    }
    ...
    private Map<Long, RoomBaseInfo> getRoomBaseInfoMap(List<Long> roomIds, Long uid) {
        ...
        Map<Long, Room> roomMap = Optional.ofNullable(roomCache.getBatch(roomIds)).orElse(new HashMap<>());
        // 房间根据好友和群组类型分组
        // 防御：排除可能为 null 的 Room 对象，避免在引用其方法时触发 NPE
        Map<Integer, List<Long>> groupRoomIdMap = roomMap.values().stream().filter(Objects::nonNull)
                .collect(Collectors.groupingBy(Room::getType,
                        Collectors.mapping(Room::getId, Collectors.toList())));
        // 获取群组信息
        List<Long> groupRoomId = Optional.ofNullable(groupRoomIdMap.get(RoomTypeEnum.GROUP.getType())).orElse(Collections.emptyList());
        Map<Long, RoomGroup> roomInfoBatch = CollectionUtil.isEmpty(groupRoomId) ? new HashMap<>() : Optional.ofNullable(roomGroupCache.getBatch(groupRoomId)).orElse(new HashMap<>());
        // 获取好友信息
        List<Long> friendRoomId = Optional.ofNullable(groupRoomIdMap.get(RoomTypeEnum.FRIEND.getType())).orElse(Collections.emptyList());
        Map<Long, User> friendRoomMap = CollectionUtil.isEmpty(friendRoomId) ? new HashMap<>() : Optional.ofNullable(getFriendRoomMap(friendRoomId, uid)).orElse(new HashMap<>());

        // 构建最终 map 时也排除 null 的 room
